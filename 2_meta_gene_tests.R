...
# schema and mpc genes for filling in 
schema_genes = read.delim("/scratch/c.c21070635/2_meta-analysis/data/SCHEMA_gene_results_paper_with_ids.tsv")[c('gene_id','gene_symbol')] %>%
  filter(!(gene_id %in% c("ENSG00000119772","ENSG00000168769","ENSG00000171456")))
mpc2_genes = read.table('/scratch/c.c21070635/2_meta-analysis/data/MPC2_genes.tsv', header=TRUE) %>%
  rename(gene_id = Gene.ID) %>%
  filter(!(gene_id %in% c("ENSG00000119772","ENSG00000168769","ENSG00000171456")))
mpc3_genes = read.table('/scratch/c.c21070635/2_meta-analysis/data/MPC3_genes.tsv', header=TRUE) %>%
  rename(gene_id = Gene.ID) %>%
  filter(!(gene_id %in% c("ENSG00000119772","ENSG00000168769","ENSG00000171456")))

# AC list and no list for filling in NAs after reshape
...
for (variant in variant_list) {
  ...
  # filling in genes after reshaping since i've got to fill in the NAs anyway  
  if (variant == "ptv") { 
  ...
  # can't just filter for CC columns because might exclude in_house_male and female 
...
# checking for number of non-0 strata 
...
for (variant in c("ptv","ptv_mpc3","ptv_mpc2","mpc2")) { 
  ...
  # holdover which might still be useful 
  meta[[variant]] = meta[[variant]] %>% 
  ...
  # using NAs for fails still here (for ease of condensing) - 
  # use dashes in supplement 
  
  # SCHEMA discovery - OR and p-val
  meta[[variant]]$schema_cmh_or = apply(meta[[variant]][,schema_strata_list], 1, 
                                             function(x) 
                                               if (pass_2sa_schema(x)) { 
                                                 mantelhaen.test(array(x,c(2,2,11)), alternative = "two.sided")[[5]][[1]] 
                                               } else {
                                                 NA 
                                               }, 
                                             simplify=TRUE)
  meta[[variant]]$schema_cmh_or_lower = apply(meta[[variant]][,schema_strata_list], 1, 
                                                   function(x) 
                                                     if (pass_2sa_schema(x)) { 
                                                       mantelhaen.test(array(x,c(2,2,11)), alternative = "two.sided")[[4]][[1]] 
                                                     } else {
                                                       NA 
                                                     }, 
                                                   simplify=TRUE)
  meta[[variant]]$schema_cmh_or_upper = apply(meta[[variant]][,schema_strata_list], 1, 
                                                   function(x) 
                                                     if (pass_2sa_schema(x)) { 
                                                       mantelhaen.test(array(x,c(2,2,11)), alternative = "two.sided")[[4]][[2]] 
                                                     } else {
                                                       NA 
                                                     }, 
                                                   simplify=TRUE)
  meta[[variant]]$schema_cmh_p_val = apply(meta[[variant]][,schema_strata_list], 1, 
                                                function(x) 
                                                  if (pass_2sa_schema(x)) { 
                                                    mantelhaen.test(array(x,c(2,2,11)), alternative = "two.sided")[[3]][[1]] 
                                                  } else {
                                                    NA 
                                                  }, 
                                                simplify=TRUE) 
    ...
    apply(meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), in_house_nonpar_strata_list], 1, 
          function(x) 
            if (in_house_nonpar_count_non_0_strata(x) == 2) {
              mantelhaen.test(array(x,c(2,2,2)), alternative = "two.sided")[[5]][[1]] 
              # , simplify=TRUE
            ...
            } else { #if (in_house_nonpar_count_non_0_strata(x) == 0)
              fisher.test(matrix(x[1:4], nrow=2), alternative = "two.sided")[[3]]
            }
    )
  meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), "in_house_cmh_lower_or"] = 
    apply(meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), in_house_nonpar_strata_list], 1, 
          function(x) 
            if (in_house_nonpar_count_non_0_strata(x) == 2) {
              mantelhaen.test(array(x,c(2,2,2)), alternative = "two.sided")[[4]][[1]] 
              # , simplify=TRUE
            } else if (in_house_nonpar_count_non_0_strata(x) == 1) {
              if (in_house_nonpar_first_non_0_strata(x)) {
  ...
  meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), "in_house_cmh_upper_or"] = 
    apply(meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), in_house_nonpar_strata_list], 1, 
          function(x) 
            if (in_house_nonpar_count_non_0_strata(x) == 2) {
              mantelhaen.test(array(x,c(2,2,2)), alternative = "two.sided")[[4]][[2]] 
              # , simplify=TRUE
            } else if (in_house_nonpar_count_non_0_strata(x) == 1) {
              if (in_house_nonpar_first_non_0_strata(x)) {
  ...
  meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), "in_house_cmh_p_val"] = 
    apply(meta[[variant]][(meta[[variant]]$gene_id %in% nonpar_genes), in_house_nonpar_strata_list], 1, 
          function(x) 
            if (in_house_nonpar_count_non_0_strata(x) == 2) {
              mantelhaen.test(array(x,c(2,2,2)), alternative = "two.sided")[[3]][[1]] 
              # , simplify=TRUE
            } else if (in_house_nonpar_count_non_0_strata(x) == 1) {
              if (in_house_nonpar_first_non_0_strata(x)) {
    ...
    apply(meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), autosomal_strata_list], 1, 
          function(x) 
            if (pass_2sa_autosomal(x)) { 
              mantelhaen.test(array(x,c(2,2,12)), alternative = "two.sided")[[5]][[1]] 
            } else {
              NA 
            }, 
          simplify=TRUE) 
  meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), "cmh_or_lower"] = 
    apply(meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), autosomal_strata_list], 1, 
          function(x) 
            if (pass_2sa_autosomal(x)) { 
              mantelhaen.test(array(x,c(2,2,12)), alternative = "two.sided")[[4]][[1]] 
            } else {
              NA 
            }, 
          simplify=TRUE) 
  meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), "cmh_or_upper"] = 
    apply(meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), autosomal_strata_list], 1, 
          function(x) 
            if (pass_2sa_autosomal(x)) { 
              mantelhaen.test(array(x,c(2,2,12)), alternative = "two.sided")[[4]][[2]] 
            } else {
              NA 
            }, 
          simplify=TRUE) 
  meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), "cmh_p_val"] = 
    apply(meta[[variant]][!(meta[[variant]]$gene_id %in% nonpar_genes), autosomal_strata_list], 1, 
          function(x) 
            if (pass_2sa_autosomal(x)) { 
              mantelhaen.test(array(x,c(2,2,12)), alternative = "two.sided")[[3]][[1]] 
            } else { 
              NA 
            }, 
          simplify=TRUE) 
    ...
    apply(meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, nonpar_strata_list], 1, 
          function(x) 
            if (pass_2sa_nonpar(x)) { 
              mantelhaen.test(array(x,c(2,2,13)), alternative = "two.sided")[[5]][[1]] 
            } else {
              NA 
            }, 
          simplify=TRUE) 
  meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, "cmh_or_lower"] = 
    apply(meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, nonpar_strata_list], 1, 
          function(x) 
            if (pass_2sa_nonpar(x)) { 
              mantelhaen.test(array(x,c(2,2,13)), alternative = "two.sided")[[4]][[1]] 
            } else {
              NA 
            }, 
          simplify=TRUE) 
  meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, "cmh_or_upper"] = 
    apply(meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, nonpar_strata_list], 1, 
          function(x) 
            if (pass_2sa_nonpar(x)) { 
              mantelhaen.test(array(x,c(2,2,13)), alternative = "two.sided")[[4]][[2]] 
            } else {
              NA 
            }, 
          simplify=TRUE) 
  meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, "cmh_p_val"] = 
    apply(meta[[variant]][meta[[variant]]$gene_id %in% nonpar_genes, nonpar_strata_list], 1, 
          function(x) 
            if (pass_2sa_nonpar(x)) { 
              mantelhaen.test(array(x,c(2,2,13)), alternative = "two.sided")[[3]][[1]] 
            } else {
              NA
            }, 
          simplify=TRUE) 
...
for (variant in c('ptv','ptv_mpc3','ptv_mpc2','mpc2')) {
  ...
  cat(paste('P-vals below 0.01: ', nrow(meta[[variant]] %>% filter(cmh_p_val <= 0.01)), '\n', sep=''))
  ...
  # for schema + de novo 
  # number of cmh p-vals below 0.01 
  #cat(paste('P-vals below 0.01: ', nrow(meta[[variant]] %>% filter(schema_cmh_p_val <= 0.01)), '\n', sep=''))
...
#View(discovery[[variant]]) 

#ptv = read.table("/scratch/c.c21070635/2_meta-analysis/output/3_meta-analysis/2_meta_rvas/2_meta_gene_results_ptv.tsv",
#                 header=TRUE) #_dedup
...
#      0,0,8866,42626,
#      0,0,7273,11182)
#schema_count_non_0_strata(x)
#mantelhaen.test(array(x,c(2,2,7)), alternative = "two.sided")[[3]][[1]] 

#x = c(2.4513e-11, 2.6e-5)
#min(x[1],sumlog(x)[[3]])
